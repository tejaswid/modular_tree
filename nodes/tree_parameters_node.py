...
class MtreeParameters(Node, BaseNode):
    """
    This is the Tree parameters Node that controls the overall generation of the tree.
    """
    ...
    create_leafs = BoolProperty(default=False, update=BaseNode.property_changed,
                                description="Toggle creation of leaves")
    # Amount of leaves to create
    leaf_amount = IntProperty(min=1, default=3000, update=BaseNode.property_changed,
                              description="Amount of leaves to create. min=1, max=inf, default=3000")
    # Maximum radius of leaf
    leaf_max_radius = FloatProperty(min=0, default=.1, update=BaseNode.property_changed,
                                    description="maximum radius of a leaf. min=0, default=0.1")
    # The leaf object
    leaf_dupli_object = PointerProperty(type=bpy.types.Object, name="leaf", update=BaseNode.property_changed,
                                        description="The object that has to be used as a leaf")
    ...
    # Boolean indicating if armature has to be created
    create_armature = BoolProperty(default=False, update=BaseNode.property_changed,
                                   description="Toggle creation of armature")
    # Minimum radius of armature
    armature_min_radius = FloatProperty(min=0, max=1, default=.08, update=BaseNode.property_changed,
                                        description="Minimum radius of armature")

    # has there been a change in the node tree or any of its parameters
    has_changed = BoolProperty(default=True)
    # reference to last tree made, used when updating
    active_tree_object = PointerProperty(type=bpy.types.Object)
    # this property is not be in the properties variable in order to avoid loop
    last_execution_info = StringProperty()

    properties = ["resolution", "create_leafs", "leaf_amount", "leaf_max_radius", "leaf_weight",
                  "leaf_dupli_object", "leaf_size", "leaf_extremity_only", "leaf_flatten",
                  "leaf_spread", "mesh_type", "create_armature", "armature_min_radius"]
    ...
    def execute(self):
        """
        The function that is called when the tree has to be generated.

        This is the function that passes the parameters from the UI to the underlying tree generation functions.
        """
        # get string of all parameters organised in a dictionary
        new_execution_info = self.id_data.save_as_json(return_string=True)
        # Get the string indicating at which level of the tree a change has been made
        ...
        # if active tree has been deleted
        if self.active_tree_object is not None and len(self.active_tree_object.users_scene) == 0:
            self.active_tree_object = None
            change_level = "tree_execution"

        tree_ob, leaf_ob, armature_ob = get_current_object("MESH" if self.mesh_type == "final"
                                                           else "CURVE", active_tree=self.active_tree_object)
        ...
        # if auto_update is on, the active object shouldn't change. If the active object changes, stop auto execution
        if self.auto_update and tree_ob is None:
        ...
        try:
            ...
            show_message_box("The tree needs a trunk node in order to execute", "Invalid node tree", "ERROR")
            return
        t0 = time.clock()
        ...
        if self.create_leafs:
            if change_level in {"tree_execution", "leafs_emitter"}:
                leaf_ob = generate_leafs_object(tree, self.leaf_amount, self.leaf_weight,
                                                self.leaf_max_radius, self.leaf_spread, self.leaf_flatten,
                                                self.leaf_extremity_only, leaf_ob, tree_ob)
            create_particle_system(leaf_ob, self.leaf_amount, self.leaf_dupli_object, self.leaf_size)
        elif leaf_ob is not None:   # if there should not be leafs yet an emitter exists, delete it
    ...
    def draw_buttons(self, context, layout):
        ...
        # Check if active_tree exists
        active_tree = self.active_tree_object is not None and len(self.active_tree_object.users_scene) > 0
        ...
        if need_update and not self.auto_update:    # if tree needs to be updated
            ...
            # if a known tree is selected
            if active_tree or ob is not None and ob.select_get() and ob.get("is_tree") is not None:
                prop_text = "Update Tree"
            else:
                prop_text = "Create Tree"
            op = layout.operator("object.mtree_execute_tree", text=prop_text)   # calls ExecuteMtreeNodeTreeOperator.execute
        ...
        op = layout.operator("mtree.randomize_tree", text='randomize tree')     # calls RandomizeTreeOperator.execute
        op.node_group_name = self.id_data.name  # set name of node group to operator


def get_current_object(tree_ob_type, active_tree):
    """
    Returns active object if it is a valid tree and potentially the leaf emitter attached to it
...
def generate_tree_object(ob, tree, resolution, tree_property="is_tree"):
    ...
    if ob is None:  # if no object is specified, create one
...
def generate_curve_object(ob, tree):
    """
    Creates the tree object as a curve
...
def generate_leafs_object(tree, number, weight, max_radius, spread, flatten, extremity_only, ob=None, tree_ob=None):
    ...
    # emitter mesh data
    verts, faces = tree.get_leaf_emitter_data(number, weight, max_radius, spread, flatten, extremity_only)
    mesh.from_pydata(verts, [], faces)  # fill object mesh with new data
    ob.parent = tree_ob     # make sure leafs emitter is child of tree object
    return ob
...
def get_tree_changes_level(last_execution_info, new_execution_info):
    """
    Compares two execution info and returns a string indicating the level of difference.

    :param last_execution_info: The older execution info
    :param new_execution_info: The newer execution info
    :return: "tree_execution", "leafs_emitter" or "particle_system" if differences are found else None
    """
    ...
    # if the node tree has changed then the tree mesh must be recreated
    if e1.keys() != e2.keys() or len(n1) != len(n2):
        return "tree_execution"

    leaf_emitter = False        # set to True if a change of level leaf emitter is detected
    particle_system = False     # set to True if a change of level particle system is detected

    # loop over each key in the node tree of the first execution info
    for key in range(len(n1)):
        # get the values at that key
        node_1 = n1[key]
        node_2 = n2[key]
        for prop in node_1.keys():
            if node_1[prop] != node_2[prop]:    # if a property has been changed
                print(prop)
                # if the property has been changed in a tree function, the tree mesh must be recreated
                if node_1["bl_idname"] != "MtreeParameters":
                    return "tree_execution"
                elif prop in {"mesh_type", "resolution", "create_armature", "armature_min_radius"}:
                    return "tree_execution"
                elif prop in {"create_leafs", "leaf_max_radius", "leaf_amount", "leaf_weight",
                              "leaf_spread", "leaf_flatten", "leaf_extremity_only"}:
                    leaf_emitter = True
                else:
...
class RandomizeTreeOperator(Operator):
    ...
    def execute(self, context):
        parameters_node = [i for i in bpy.data.node_groups[self.node_group_name].nodes
                           if i.bl_idname == "MtreeParameters"][0]
